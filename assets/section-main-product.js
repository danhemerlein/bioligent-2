if (typeof ProductPage !== 'function') {
  class ProductPage extends HTMLElement {
    constructor() {
      ...
      this.showMiniCart = this.dataset.openMiniCartAtc
      console.log('this.showMiniCart', this.showMiniCart)
      console.log('this.showMiniCart === true', this.showMiniCart === 'true')

      this.pickupAvailabilityExtended = this.querySelector(
      ...
      if (this.productSlider) {
        const productGaleryThumbnails = this.querySelector(
          ...
          this.querySelectorAll(
            ...
            elm.addEventListener('click', (e) => {
              if (this.productSlider.sliderEnabled) {
                ...
                window.scrollTo({
                  top: this.productGallery
                    .querySelector(
                      `.product-gallery-item[data-index="${e.currentTarget.dataset.index}"]`
                    )
              ...
              this._pauseAllMedia()
              this._playMedia(
                this.productGallery.querySelector(
                  `.product-gallery-item[data-index="${e.currentTarget.dataset.index}"]`
                )
              )
        ...
        this.productSlider.addEventListener('navigation', (e) => {
          this._playMedia(
            this.productGallery.querySelector(
              `.product-gallery-item[data-index="${e.target.index}"]`
            )
          )
        })
        this.productSlider.addEventListener('change', (e) => {
          ...
          if (
            this.productGallery.querySelector(
              `.product-gallery-item[data-index="${e.target.index}"]`
            ).dataset.productMediaType == 'model'
          ) {
            if (this.xrButton) {
              this.xrButton.setAttribute(
            ...
            setTimeout(() => {
              this.productSlider
                .querySelector('.css-slider-holder')
                .classList.add('css-slider--disable-dragging')
            }, 150)
          }
        })
        ...
        this.productSlider.addEventListener('ready', (e) => {
          if (this.firstProductGalleryIndex) {
            this.productSlider.changeSlide(
              this.firstProductGalleryIndex,
              'auto'
            )
          }
        })
      ...
      this.productVariants = this.querySelector(
        'product-variants[data-main-product-variants]'
      )
      if (this.productVariants) {
        this.productVariants.addEventListener(
          'VARIANT_CHANGE',
          this.onVariantChangeHandler.bind(this)
        )
        this.onVariantChangeHandler({ target: this.productVariants })

        // refresh color
        this.querySelectorAll('.product-variant__item--radio input').forEach(
          (input) => {
            input.addEventListener('click', () => {
              const inputParent = input.closest('.product-variant__item--radio')
              if (
                inputParent.classList.contains('product-variant__item--color')
              ) {
                inputParent
      ...
      if (
        ...
        KROWN.settings.cart_action == 'overlay'
      ) {
        let addToCartEnter = false
        if (this.querySelector('[data-js-product-add-to-cart]')) {
          this.querySelector('[data-js-product-add-to-cart]').addEventListener(
            'keyup',
            (e) => {
        ...
        if (this.querySelector('[data-js-product-form]')) {
          this.querySelector('[data-js-product-form]').addEventListener(
            'add-to-cart',
            () => {
              if (this.showMiniCart === 'true') {
                document.getElementById('site-cart-sidebar').show()
              }
              if (document.getElementById('cart-recommendations-sidebar')) {
                document
                  .getElementById('cart-recommendations-sidebar')
                  .generateRecommendations()
              }
              if (addToCartEnter) {
                setTimeout(() => {
                  document
                    .querySelector('#site-cart-sidebar [data-js-close]')
                    .focus()
                }, 200)
              }
            }
      ...
      if (addToCartButton) {
        ...
        buyObserver.observe(addToCartButton, {
          attributes: true,
          childList: false,
          subtree: false,
        })
    ...
    thumbnailNavigationHelper(index = 0) {
      this.querySelectorAll('.product-gallery__thumbnails .thumbnail').forEach(
        (elm, i) => {
          if (i == index) elm.classList.add('active')
    ...
    onVariantChangeHandler(e) {
      ...
      if (variant) {
        ...
        if (variant.featured_media != null) {
          let variantImg = this.productGallery.querySelector(
            '.product-gallery-item[data-media-id="' +
              variant.featured_media.id +
              '"]'
          )
          if (variantImg) {
            if (
              this.productGallery.classList.contains(
              ...
              if (
                this.productGallery.querySelector('css-slider') &&
                this.productGallery.querySelector('css-slider').sliderEnabled
              ) {
                this.productGallery
                  .querySelector('css-slider')
                  .changeSlide(variantImg.dataset.index)
              } else {
                this.firstProductGalleryIndex = variantImg.dataset.index
              }
            } else {
              if (variantImg.getBoundingClientRect().top > window.innerHeight) {
                variantImg.scrollIntoView({
                  behavior: 'smooth',
                  block: 'center',
                })
        ...
        this.querySelectorAll(
          '.product-variant__item--color [data-selected]'
        ).forEach((elm) => {
          elm
            .closest('.product-variant')
            .querySelector('.product-variant__item-text-label').textContent =
            elm.value
        })
      }
    }
    ...
    _pauseAllMedia() {
      document
        .querySelectorAll('.product-gallery .js-youtube')
        .forEach((video) => {
      ...
      document
        .querySelectorAll('.product-gallery .js-vimeo')
        .forEach((video) => {
          video.contentWindow.postMessage('{"method":"pause"}', '*')
        })
      document
        .querySelectorAll('.product-gallery video')
        .forEach((video) => video.pause())
      document
        .querySelectorAll('.product-gallery product-model')
        .forEach((model) => {
